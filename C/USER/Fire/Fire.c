...
void Fire_Receiving_process(void)
{
  ...
  packet_head_t packet;
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        uint32_t temp_p = COMPOUND_32BIT(&frame_data[13]);
        uint32_t temp_i = COMPOUND_32BIT(&frame_data[17]);
        uint32_t temp_d = COMPOUND_32BIT(&frame_data[21]);
        
        if (packet.ch == CURVES_CH1)
        {
					Fire.PID1_P = *(float *)&temp_p;
					Fire.PID1_I = *(float *)&temp_i;
					Fire.PID1_D = *(float *)&temp_d;
        }
        else if (packet.ch == CURVES_CH2)
        {
					Fire.PID2_P = *(float *)&temp_p;
					Fire.PID2_I = *(float *)&temp_i;
					Fire.PID2_D = *(float *)&temp_d;
        }
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        Fire.Target_value =  actual_temp;         // 设置目标值
      }
      break;
...
uint8_t Fire_data[256];

void Fire_uart_init(void)
{
	__HAL_UART_ENABLE_IT(&UartHandle, UART_IT_RXNE);  //接收中断
  __HAL_UART_ENABLE_IT(&UartHandle, UART_IT_IDLE);  //空闲中断
	
	HAL_UART_Receive_DMA(&UartHandle,Fire_data,256);//开始接收（使用DMA）
}
