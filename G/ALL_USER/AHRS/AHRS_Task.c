...
static TaskHandle_t Start_Task_IMU_local_handler;
...
imu_type IMU;//陀螺仪YAW PITCH ROLL角度

void AHRS_Init(void)//用户读取IMU数据初始化
{
...
//		usart_printf("a = %c\r\n",ist8310_init());
	...
	//获取当前任务的任务句柄，
	Start_Task_IMU_local_handler = xTaskGetHandle(pcTaskGetName(NULL));//
...
//					usart_printf("b\r\n");
        ...
        get_angle(INS_quat, INS_angle + INS_YAW_ADDRESS_OFFSET, INS_angle + INS_PITCH_ADDRESS_OFFSET, INS_angle + INS_ROLL_ADDRESS_OFFSET);
				...
				IMU.Last_Read_Yaw = IMU.Read_Yaw;
				IMU.Read_Yaw = INS_angle[0] * (180/PI);
				IMU.Send_Yaw = INS_angle[0] * (180/PI);
				IMU.Pitch = INS_angle[2] * (180/PI);
				IMU.Roll = INS_angle[1] * (180/PI);
				IMU.Temp = bmi088_real_data.temp;
				IMU.gyro_x = INS_gyro[0] * 100;IMU.gyro_y = INS_gyro[1] * 100;IMU.gyro_z = INS_gyro[2] * 100;
				IMU.acc_x = INS_accel[0];IMU.acc_y = INS_accel[1];IMU.acc_z = INS_accel[2];
				IMU.q0 = INS_quat[0];IMU.q1 = INS_quat[1];IMU.q2 = INS_quat[2];IMU.q3 = INS_quat[3];

				if(IMU.Read_Yaw < 0)  
					IMU.Read_Yaw = IMU.Read_Yaw + 360;    
      	if(IMU.Read_Yaw > 360)
					IMU.Read_Yaw = IMU.Read_Yaw - 360;
					
				if((IMU.Read_Yaw - IMU.Last_Read_Yaw) > 180)
					IMU.Circle--;
				else if((IMU.Read_Yaw - IMU.Last_Read_Yaw) < -180)
					IMU.Circle++;
						
				IMU.Yaw = IMU.Circle*360 + IMU.Read_Yaw;
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        PID_calc(&imu_temp_pid, temp, EXCEPT_TEMPERATURE);//get_control_temperature()
    ...
    {        
			...
			if (temp > EXCEPT_TEMPERATURE)//get_control_temperature()
			{
					temp_constant_time++;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    {
//        detect_hook(BOARD_ACCEL_TOE);//用于检测模块是否正常工作，从而记录状态
...
//        detect_hook(BOARD_MAG_TOE);
    ...
    else if(GPIO_Pin == GPIO_PIN_0)
    {
        ...
        if (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)
        {
            ...
            vTaskNotifyGiveFromISR(Start_Task_IMU_local_handler, &xHigherPriorityTaskWoken);
