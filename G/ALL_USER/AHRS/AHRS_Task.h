...
#define EXCEPT_TEMPERATURE 45.0f      //期望温度
...
typedef __packed struct
{
  ...
  float Read_Yaw;
	...
	float Last_Read_Yaw;
	float Yaw;
	float gyro_x;      //陀螺仪加速度数据
	float gyro_y;      //陀螺仪加速度数据
	float gyro_z;      //陀螺仪加速度数据
	float acc_x;
	float acc_y;
	float acc_z;
	float q0;           //四元素
...
extern imu_type IMU;//陀螺仪YAW PITCH ROLL角度
/**
...
extern void AHRS_Init(void);//陀螺初始化
extern void AHRS_Read(void);//陀螺仪读取数据
#endif
