...
//#define cali_get_mcu_temperature()  get_temprate()     //没用ADC,用ADC才用这个 
...
typedef __packed struct
{
	...
	//数据类型为 bool_t => unsigned char；对应的是一个(*cali_hook)的指针；(uint32_t *point, bool_t cmd)函数需要两个参数
} cali_sensor_t;
...
extern void Calibrate_AHRS(void);//校准imu用户调用函数
#endif
