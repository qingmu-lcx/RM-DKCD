...
void Calibrate_AHRS(void)
{
	...
	{
			if (cali_sensor[i].cali_cmd)// ＝ 1就是说明利用遥控器校准成功了
			{
...
//    static const uint8_t CHASSIS_FLAG = 4;
    ...
    else if (rc_action_flag == 3 && rc_cmd_time > RC_CMD_LONG_TIME)
    {
        ...
        rc_action_flag = 0;
        rc_cmd_time = 0;
        cali_sensor[CALI_GYRO].cali_cmd = 1;//当遥控器调到对应位置时，重点改变这个 
				/*
				//update control temperature
        head_cali.temperature = (int8_t)(cali_get_mcu_temperature()) + 10;
				//校准的时候，读取当前陀螺仪温度，并将当前温度加10后的值作为PID期望控温，但是暂时没用，用的是直接给定温度，进行PID控温
...
/*判断当前遥控器状态*/
    if (RC.l_x > 1600 && RC.l_y < 450 && RC.r_x < 450 && RC.r_y < 450 && RC.l_s == 1)
    {
        //two rockers set to  \../, hold for 2 seconds,
        //两个摇杆打成 \../,保持2s
        rc_cmd_time++;
    }
    else if (RC.l_x < 450 && RC.l_y < 450 && RC.r_x > 1600 && RC.r_y < 450 && RC.l_s == 1)
    {
        //two rocker set to ./\., hold for 2 seconds
        //两个摇杆打成./\.,保持2s
        rc_cmd_time++;
        rc_action_flag = GYRO_FLAG;//3
    }
    else
    {
        rc_cmd_time = 0;
...
/**
  ...
  */
/**
  * @brief          使用遥控器开始校准，例如陀螺仪，云台，底盘(刘：初始化陀螺仪，从flash中读取校准值)
  * @param[in]      none
  * @retval         none
  */
void Cali_Param_Init(void)
{
    ...
    for (i = 0; i < CALI_LIST_LENGHT; i++)
    {
        cali_sensor[i].flash_len = cali_sensor_size[i];//获取数据大小
        cali_sensor[i].flash_buf = cali_sensor_buf[i];//获取对应结构体的数据 cali data address
        cali_sensor[i].cali_hook = (unsigned char(*)(uint32_t *, unsigned char))cali_hook_fun[i];//返回0或1 若校准完毕则返回1
    }

    cali_data_read();//从flash读取校准数据
...
static void cali_data_read(void)
{
    ...
    for (i = 0; i < CALI_LIST_LENGHT; i++)
    {
        ...
        {
            cali_sensor[i].cali_cmd = 1;////////////////////////////////////用遥控器校准的时候需要把这个屏蔽了
        }
    }
}
