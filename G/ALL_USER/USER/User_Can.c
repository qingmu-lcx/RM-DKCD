...
#include "Gimbal.h"
/*
CAN1 上有两个3508摩擦轮，Roll轴、Pitch轴、Yaw轴，2006电机
CAN2 上有底盘四个电机
*/
//判断CAN1接收到的信息来自于底盘电机
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)//接收回调函数
{
	...
	{	    
		...
		else if(can1_gimbal_motor(rx_message.StdId))	//接收云台电机数据
		{
			...
			if(Yaw_Position_Init-CAN1_Motor[id].Angle < 4100)//-2500是6100-8192=-2092 < 4100 > -4100
			{
				CAN1_Motor[id].diversity_Angle=Yaw_Position_Init-CAN1_Motor[id].Angle;
			}
			else
			{
				CAN1_Motor[id].diversity_Angle=Yaw_Position_Init-CAN1_Motor[id].Angle+8192;
			} 
								 /*Angle_Radian 弧度角*/
			CAN1_Motor[id].Angle_Radian=Yaw_Position_Init-CAN1_Motor[id].Angle>=0?
			Yaw_Position_Init-CAN1_Motor[id].Angle:8192+Yaw_Position_Init-CAN1_Motor[id].Angle;//3436+8192   //8912
									/*Circle圈数计算·*/
			...
			if(CAN1_Motor[id].Flag==0)    //整个程序只执行一次，用于判断电机的初始值
			{
				...
				CAN1_Motor[id].Angle_Difference=(Yaw_Position_Init-CAN1_Motor[id].Angle);//
			}            
		}
		else
		{
//      Receive_Data(&rx_message,rx_data);
	...
	{	    
		if ( HAL_OK == HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &rx_message, rx_data))//获取接收完成标志
		{
			...
			//在这里解析can2接收数据
			Rx_Chassis_To_Gimbal(&rx_message,rx_data);
